...
public class LimelightPoseEstimator
{
   ...
   * <p>
   * Each mode corresponds to a different vision pipeline for AprilTag-based localization, with distinct methods for
   * combining visual and robot data.
   */
  public static enum EstimationMode
  {
     ...
     * <p>
     * MegaTag1 estimates the robot's pose using data from a single detected AprilTag. It relies entirely on visual
     * information from the camera, without using any external robot state data.
     */
    MEGATAG1,
     ...
     * <p>
     * MegaTag2 fuses AprilTag data with the robot's current heading (from a gyro or odometry source) to refine the
     * estimated pose. This results in smoother and more accurate global localization, especially when tags are viewed
     * from oblique angles or at long range.
     *
  ...
  public enum BotPose
  {
    ...
    {
      var hashKey = camera.limelightName+entry;
      if (!poseEstimate.containsKey(hashKey))
      {poseEstimate.putIfAbsent(hashKey, new PoseEstimate(camera, entry, isMegaTag2));}
      return poseEstimate.get(hashKey).getPoseEstimate();
    }
